...
function StudentResult() {
  ...
  useEffect(() => {
    const getClassrooms = async () => {
      try {
        const response = await dispatch(syllabusList());
        if (!response.error) {
                  ...
                  >
                    ...
                    {classrooms.map((classroom) => (
                      <option key={classroom.id} value={classroom.id}>
                        {classroom.classroom.class_no} {classroom.classroom.section} - {classroom.subject.subject} - {classroom.teacher.username}
                      </option>
                    ))}
                  </select>
                </div>
